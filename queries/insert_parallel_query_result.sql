CREATE TABLE IF NOT EXISTS big_point_data (
  sub_polygon_id INTEGER,
  band_number INTEGER,
  pixel_center_coordinates TEXT,
  pixel_value DOUBLE PRECISION,
  pixel_column INTEGER,
  pixel_row INTEGER
);
...
--         11.157073742 60.754691924))',
  ...
  LOOP
    full_query := full_query || FORMAT($f$
      INSERT INTO point_data (sub_polygon_id, band_number, pixel_center_coordinates, pixel_value, pixel_column, pixel_row)
      SELECT
        %s AS sub_polygon_id,
